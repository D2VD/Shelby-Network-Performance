"use client";
/**
 * app/dashboard/charts/page.tsx — v16.0  (also used by app/charts/page.tsx via re-export)
 * FIXES:
 * 1. Global Run History: show ALL runs from DB, no deployedAtMs filter
 * 2. Storage Analytics: add Avg Blob Size chart
 * 3. Avg Blob Size = totalStorageBytes / activeBlobs (accurate: same source)
...
interface TsPoint {
  tsMs: number; activeBlobs: number; totalStorageGB: number; totalStorageBytes?: number;
  totalBlobEvents: number; pendingOrFailed: number; deletedBlobs: number; blockHeight: number;
  avgBlobSizeKB?: number; // computed: totalStorageBytes / activeBlobs / 1024
}
interface LivePt {
...
interface ServerBench {
  ...
  avgUploadKbs: number; avgDownloadKbs: number;
  latencyAvg: number; txConfirmMs: number;
...
function Chart({ series, labels, height=150, perScale=false }: { series:Series[]; labels:string[]; height?:number; perScale?:boolean; }) {
  ...
  const fY=(v:number)=>{ if(!isFinite(v)||v===0)return""; if(v>=1e9)return`${(v/1e9).toFixed(1)}G`; if(v>=1e6)return`${(v/1e6).toFixed(1)}M`; if(v>=1e3)return`${(v/1e3).toFixed(0)}K`; return String(Math.round(v)); };
  const toIdx=(clientX:number)=>{ const el=svgRef.current; if(!el)return 0; const rect=el.getBoundingClientRect(); const frac=(clientX-rect.left)/rect.width; const raw=(frac*VW-PL)/iW*(n-1); return Math.max(0,Math.min(n-1,Math.round(raw))); };
  ...
  const tipR=active!==null?active<n*0.55:true,tipX=active!==null?(xp(active)/VW*100):50;
  ...
  return(
    <div style={{position:"relative"}} onMouseEnter={()=>setIn(true)} onMouseLeave={()=>{setIn(false);setHIdx(null);}}>
      <svg ref={svgRef} viewBox={`0 0 ${VW} ${height}`} style={{width:"100%",height,display:"block",cursor:"crosshair"}}
        onMouseMove={e=>{if(!in_)return;setHIdx(toIdx(e.clientX));}} onMouseLeave={()=>setHIdx(null)}
        ...
        <defs>{series.map((s,si)=><linearGradient key={si} id={`g${si}${s.color.replace(/[^a-z0-9]/gi,"")}`} x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor={s.color} stopOpacity={isDark?0.35:0.2}/><stop offset="100%" stopColor={s.color} stopOpacity={0.02}/></linearGradient>)}</defs>
        {series.map((s,si)=>{ if(s.data.length<2)return null; const pts=s.data.map((v,i)=>`${xp(i).toFixed(1)},${yp(v,si).toFixed(1)}`).join(" "); const area=`${xp(0).toFixed(1)},${PT+iH} ${pts} ${xp(s.data.length-1).toFixed(1)},${PT+iH}`; return<g key={si}><polygon points={area} fill={`url(#g${si}${s.color.replace(/[^a-z0-9]/gi,"")})`}/><polyline points={pts} fill="none" stroke={s.color} strokeWidth={2} strokeLinejoin="round"/><circle cx={xp(s.data.length-1)} cy={yp(s.data[s.data.length-1],si)} r={4} fill={s.color} stroke={isDark?"#0f172a":"#fff"} strokeWidth={2}/></g>;})}
        {active!==null&&in_&&(()=>{const cx=xp(active);return<g><line x1={cx} y1={PT} x2={cx} y2={PT+iH} stroke={isDark?"#64748b":"#94a3b8"} strokeWidth={1} strokeDasharray="3 3"/>{series.map((s,si)=>{const v=s.data[active];if(v==null||!isFinite(v))return null;return<circle key={si} cx={cx} cy={yp(v,si)} r={5} fill={s.color} stroke={isDark?"#0f172a":"#fff"} strokeWidth={2}/>;})} </g>;})()}
        {labels.length>0&&[0,Math.floor(labels.length/2),labels.length-1].map(i=>labels[i]?<text key={i} x={xp(i)} y={height-4} textAnchor="middle" fontSize={9} fill={tc}>{str(labels[i])}</text>:null)}
      </svg>
      {active!==null&&in_&&<div style={{position:"absolute",left:tipR?`${tipX+1}%`:"auto",right:tipR?"auto":`${100-tipX+1}%`,top:4,background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:9,padding:"9px 13px",fontSize:12,pointerEvents:"none",zIndex:50,minWidth:140,whiteSpace:"nowrap",boxShadow:"0 4px 14px var(--shadow-color)"}}>
        ...
        {series.map((s,si)=>{const v=s.data[active];if(v==null||!isFinite(v))return null;return<div key={si} style={{display:"flex",justifyContent:"space-between",gap:12,marginBottom:1}}><span style={{color:"var(--text-muted)"}}>{str(s.name)}</span><span style={{fontWeight:700,fontFamily:"monospace",color:s.color}}>{str(s.fmt?s.fmt(v):fmtN(v))}</span></div>;})}
      </div>}
    </div>
  );
}
...
function Sec({title,sub,children,right}:{title:string;sub?:string;children:React.ReactNode;right?:React.ReactNode}) {
  return<div style={{marginBottom:32}}><div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:8}}><div><h2 style={{fontSize:19,fontWeight:800,color:"var(--text-primary)",margin:0}}>{str(title)}</h2>{sub&&<p style={{fontSize:13,color:"var(--text-muted)",margin:"3px 0 0"}}>{str(sub)}</p>}</div>{right&&<div>{right}</div>}</div>{children}</div>;
}
function Card({title,sub,latest,color,children}:{title:string;sub?:string;latest?:string;color?:string;children:React.ReactNode}) {
  return<div style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:13,padding:"16px 18px"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}><div><div style={{fontSize:13,fontWeight:700,color:"var(--text-secondary)"}}>{str(title)}</div>{sub&&<div style={{fontSize:11,color:"var(--text-dim)"}}>{str(sub)}</div>}</div>{latest&&<div style={{fontFamily:"monospace",fontSize:15,fontWeight:800,color:str(color)||"var(--text-primary)"}}>{str(latest)}</div>}</div>{children}</div>;
}
function RangeSel({range,onChange}:{range:TimeRange;onChange:(r:TimeRange)=>void}) {
  return<div style={{display:"flex",gap:3,background:"var(--bg-card2)",border:"1px solid var(--border)",borderRadius:8,padding:3}}>{(["1h","24h","7d","30d"] as TimeRange[]).map(r=><button key={r} onClick={()=>onChange(r)} style={{padding:"5px 13px",borderRadius:6,fontSize:12,fontWeight:r===range?700:400,border:"none",cursor:"pointer",background:r===range?"var(--accent)":"transparent",color:r===range?"#fff":"var(--text-muted)",transition:"all 0.1s"}}>{r}</button>)}</div>;
}
function SnapCard({label,value,delta,from,color}:{label:string;value:string;delta:number|null;from:number|null;color?:string}) {
  ...
  return<div style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:12,padding:"14px 18px",display:"flex",flexDirection:"column",gap:4}}>
    ...
    {safeDelta!==null&&<div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}>
...
function Pager({total,page,per,set}:{total:number;page:number;per:number;set:(p:number)=>void}) {
  ...
  return<div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:5,marginTop:12}}>
    ...
    {Array.from({length:Math.min(pages,8)},(_,i)=>i).map(i=><button key={i} onClick={()=>set(i)} style={{padding:"4px 9px",borderRadius:6,border:"1px solid var(--border)",background:i===page?"var(--accent)":"var(--bg-card)",color:i===page?"#fff":"var(--text-muted)",cursor:"pointer",fontWeight:i===page?700:400,fontSize:13,minWidth:32}}>{i+1}</button>)}
...
export default function ChartsPage() {
  ...
  // Helper: compute avgBlobSizeKB from a snapshot point
  const computeAvgBlobKB = (activeBlobs: number, totalStorageGB: number): number => {
    if (activeBlobs <= 0 || totalStorageGB <= 0) return 0;
    // totalStorageGB * 1e9 bytes / activeBlobs / 1024 = KB per blob
    return (totalStorageGB * 1e9) / activeBlobs / 1024;
  };

  const enrichPoint = (s: any): TsPoint => ({
    tsMs:          num(s.tsMs),
    ...
    // Compute Avg Blob Size from the same data source (accurate)
    avgBlobSizeKB: computeAvgBlobKB(num(s.activeBlobs), num(s.totalStorageGB)),
  });
  ...
  // FIX: Fetch ALL benchmark results — no deploy filter
  ...
  useEffect(()=>{
    ...
    timerRef.current=setInterval(()=>{fetchLive(network);fetchTs48h(network);fetchBench();},POLL);
  ...
  return(
    <div style={{background:"var(--bg-primary)",minHeight:"100vh",padding:"28px 36px 48px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:14,marginBottom:14}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {[
              ...
              {label:"Avg Blob Size",  val:fmtKB(currentAvgBlobKB),          c:"var(--accent)",
               hint:"= totalStorage / activeBlobs (Shelby Indexer)"},
              ...
              <div key={label} style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:10,padding:"12px 16px",flex:1}}>
                ...
                {hint&&<div style={{fontSize:9,color:"var(--text-dim)",marginTop:3}}>{hint}</div>}
              </div>
            ))}
          </div>
        </div>

        {/* Row 2: Avg Blob Size chart — NEW */}
        {/* Algorithm:
            avgBlobSizeKB = (totalStorageGB × 1e9 bytes) / activeBlobs / 1024
            Source: both fields from Shelby Dedicated Indexer blobs_aggregate
            - totalStorageGB = sum(size) of active blobs / 1e9
            - activeBlobs = count of active blobs
            Same query, same filter (is_deleted=0, is_written=1, size>0)
            → ratio is internally consistent and accurate */}
        <Card
          title="Avg Blob Size over Time"
          sub="= Total Storage / Active Blobs · accurate when both fields share same indexer source"
          latest={fmtKB(currentAvgBlobKB)}
          color="var(--accent)"
        >
          <Chart
            series={[{
              data:   cd.map(p => num(p.avgBlobSizeKB)),
              color:  "var(--accent)",
              name:   "Avg Size",
              fmt:    v => fmtKB(v),
            }]}
      ...
      {/* Benchmark Analytics — ALL runs, no deploy filter */}
      <Sec title="Benchmark Analytics" sub={`${allBench.length} total runs · all time`}>
        {benchLoading?(
          ...
          <div style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:12,padding:"36px 20px",textAlign:"center",color:"var(--text-muted)"}}>
            ...
            <div style={{fontSize:12,marginTop:6,color:"var(--text-dim)"}}>Run a benchmark on the Benchmark page to populate</div>
          </div>
        ):(
          <>
            {/* Summary stats */}
            ...
            {/* Global Run History table — ALL runs */}
              ...
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                  <thead><tr style={{background:"var(--bg-card2)"}}>
                    {["Device","Time","Score","Tier","Upload","Download","Latency","TX Confirm","Mode"].map(h=><th key={h} style={{padding:"9px 13px",textAlign:"left",fontSize:10,fontWeight:600,color:"var(--text-dim)",textTransform:"uppercase",letterSpacing:"0.06em",whiteSpace:"nowrap",borderBottom:"1px solid var(--border)"}}>{h}</th>)}
                  </tr></thead>
                  <tbody>
                    {pagedBench.map((h,i)=>{
                      const sc=num(h.score),c=sc>=900?"#22c55e":sc>=600?"#fbbf24":"#f87171";
                      return<tr key={str(h.id)||i} style={{borderTop:"1px solid var(--border-soft)"}}>
                        <td style={{padding:"8px 13px",fontFamily:"monospace",fontSize:11,color:"var(--text-dim)"}}>{str(h.deviceId||h.ip)}</td>
                        ...
                        <td style={{padding:"8px 13px",fontFamily:"monospace",color:"#22c55e"}}>{fmtKbs(h.avgDownloadKbs)}</td>
