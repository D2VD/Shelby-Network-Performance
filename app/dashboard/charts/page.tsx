"use client";
/**
 * app/dashboard/charts/page.tsx — v24.0
 * UNIFIED layout — identical for Shelbynet and Testnet
 * FIXES:
 * 1. Data no longer jumps to 0: use timeseries as fallback for snapshot values
 * 2. Testnet has same layout as Shelbynet (Global Run History shown empty for testnet)
 * 3. LiveClock uses suppressHydrationWarning — no React #418 error
 * 4. Network snapshot uses timeseries latest point when live returns 0
 */
...
// ─── Live Clock — suppressHydrationWarning prevents #418 ─────────────────────
function LiveClock() {
  ...
  return (
    <span suppressHydrationWarning style={{ fontFamily: "monospace", fontSize: 12, color: "var(--text-dim)", background: "var(--bg-card)", border: "1px solid var(--border)", padding: "4px 10px", borderRadius: 7, display: "inline-block" }}>
      🕐 {clock}
    </span>
  );
}
...
function DeviceBadge({ h }: { h: Pick<ServerBench,"ip"|"deviceId"> }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontFamily: "monospace", fontSize: 11 }}>
      <span style={{ color: kind === "legacy" ? "#94a3b8" : "var(--text-muted)", fontStyle: kind === "legacy" ? "italic" : "normal" }}>{id}</span>
      {s.label && <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 5px", borderRadius: 4, background: s.bg, color: s.color, textTransform: "uppercase" }}>{s.label}</span>}
    </span>
  );
}
    ...
    <div style={{ position: "relative" }} onMouseEnter={() => setInChart(true)} onMouseLeave={() => { setInChart(false); setHoverIdx(null); }}>
      ...
      >
        {ticks.map(({ f, v }) => { const y = PT + iH - f*iH; return (
          <g key={f}><line x1={PL} x2={VW-PR} y1={y} y2={y} stroke={gc} strokeWidth={1} /><text x={PL-5} y={y+3} textAnchor="end" fontSize={9} fill={tc}>{fY(v)}</text></g>
        ); })}
        ...
        {series.map((s, si) => {
          ...
          return <g key={si}><polygon points={area} fill={`url(#cg${si}${s.color.replace(/[^a-z0-9]/gi,"")})`}/><polyline points={pts} fill="none" stroke={s.color} strokeWidth={2} strokeLinejoin="round"/>{s.data.length>0&&<circle cx={xp(s.data.length-1)} cy={yp(s.data[s.data.length-1],si)} r={4} fill={s.color} stroke={isDark?"#0f172a":"#fff"} strokeWidth={2}/>}</g>;
        })}
      ...
      {active !== null && inChart && (
        <div style={{ position:"absolute", left:tipRight?`calc(${xp(active)/VW*100}% + 8px)`:"auto", right:tipRight?"auto":`calc(${100-xp(active)/VW*100}% + 8px)`, top:8, zIndex:50, pointerEvents:"none", background:"var(--bg-card)", border:"1px solid var(--border)", borderRadius:10, padding:"9px 13px", fontSize:12, minWidth:150, whiteSpace:"nowrap", boxShadow:"0 4px 20px var(--shadow-color)" }}>
          {pinIdx!==null&&<div style={{fontSize:9,color:"var(--accent)",marginBottom:4,fontWeight:600}}>📌 Pinned — click to unpin</div>}
          {labels[active]&&<div style={{color:"var(--text-dim)",fontSize:10,marginBottom:6,fontWeight:600}}>{labels[active]}</div>}
          {series.map((s,si) => { const v=s.data[active]; if(!v||!isFinite(v)) return null; return <div key={si} style={{display:"flex",justifyContent:"space-between",gap:16,marginBottom:2,alignItems:"center"}}><span style={{display:"inline-flex",alignItems:"center",gap:5,color:"var(--text-muted)"}}><span style={{width:8,height:8,borderRadius:"50%",background:s.color,display:"inline-block",flexShrink:0}}/>{s.name}</span><span style={{fontWeight:700,fontFamily:"monospace",color:s.color}}>{s.fmt?s.fmt(v):fmtN(v)}</span></div>; })}
        </div>
      )}
    </div>
  );
...
function SnapCard({ label, value, delta, from, color }: { label: string; value: string; delta: number | null; from: number | null; color?: string }) {
  ...
  const pos = delta !== null ? delta > 0 : null;
    ...
    <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 12, padding: "14px 18px", display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      {delta !== null && (
        <div style={{ display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
          <span style={{ fontSize: 11, fontWeight: 600, padding: "1px 7px", borderRadius: 4, color: pos === true ? "#22c55e" : delta < 0 ? "#ef4444" : "var(--text-muted)", background: pos === true ? "rgba(34,197,94,0.1)" : delta < 0 ? "rgba(239,68,68,0.1)" : "rgba(0,0,0,0.04)" }}>
            {delta > 0 ? "+" : ""}{Math.round(delta).toLocaleString("en-US")}
          </span>
          {safeP !== null && <span style={{ fontSize: 10, color: pos === true ? "#22c55e" : delta < 0 ? "#ef4444" : "var(--text-muted)", fontWeight: 600 }}>({safeP >= 0 ? "+" : ""}{safeP.toFixed(1)}%)</span>}
...
export default function ChartsPage() {
  ...
  const [liveSnap,     setLiveSnap]     = useState<TsPoint | null>(null); // latest live point
  ...
  const fetchLive = useCallback(async (net: string) => {
    try {
      ...
      if (!d.blockHeight && !d.activeBlobs) throw new Error("Empty response");
      if (alive.current) {
        const method = String(d.method ?? "");
        setIsStale(method.includes("stale") || method.includes("cache") || method.includes("fallback"));
        setFetchError(null);
        setLiveSnap({
          tsMs:             Date.now(),
      ...
      if (alive.current) setFetchError(`Live fetch failed — using cached timeseries`);
    }
  }, []);
  ...
  useEffect(() => {
    if (alive.current) { setLiveSnap(null); setFetchError(null); setIsStale(false); }
  ...
  // FIX: Use timeseries data as fallback source of truth for current values
  // This prevents jumping to 0 when live sync temporarily fails
  const cd = ts.length > 0 ? ts : [];
  const labels  = cd.map(p => tLbl(p.tsMs, range));

  // For snapshot cards: prefer live point, fall back to latest TS point
  const latestTs = cd[cd.length - 1];

  // Merge live + ts: use live if non-zero, else use ts
  const cur = {
    ...
    <div style={{ background: "var(--bg-primary)", minHeight: "100vh", padding: "28px 36px 48px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12, marginBottom: 28 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "var(--text-muted)", margin: "4px 0 0" }}>
            {isTestnet ? `Shelby Testnet · Contract: 0x85fdb9a1…` : (config?.label ?? "Shelbynet")} · Refresh every {POLL/1000}s
          </p>
        </div>
      ...
      {(fetchError || isStale) && (
        <div style={{ background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 9, padding: "8px 14px", marginBottom: 16, fontSize: 12, color: "#d97706", display: "flex", alignItems: "center", gap: 6 }}>
          <span>⏱</span><span>{fetchError ?? "Showing cached data — live sync in progress"}</span>
      ...
      {/* ── Benchmark Analytics ─────────────────────────────────────────────── */}
      <Sec title="Benchmark Analytics" sub={isTestnet ? "Shelbynet benchmark runs (benchmarks only on Shelbynet)" : `${allBench.length} total runs · all time`}>
        {isTestnet ? (
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
              <Card title="Score History" sub="All users · all time" latest={String(allBench[0]?.score ?? "")} color="#818cf8">
                <Chart series={[{ data: benchChron.map(h => num(h.score)), color: "#818cf8", name: "Score", fmt: v => `${Math.round(v)}/1000` }]} labels={bLabels} height={130} />
              </Card>
              <Card title="Avg Upload Speed" latest={fmtKbs(avgUpload)} color={accentColor}>
                <Chart series={[{ data: benchChron.map(h => num(h.avgUploadKbs)), color: accentColor, name: "Upload", fmt: v => fmtKbs(v) }]} labels={bLabels} height={130} />
              </Card>
              <Card title="Avg Latency" sub="Node ping" latest={fmtMs(avgLatency)} color="#c084fc">
                <Chart series={[{ data: benchChron.map(h => num(h.latencyAvg)), color: "#c084fc", name: "Latency", fmt: v => fmtMs(v) }]} labels={bLabels} height={130} />
              </Card>
              <Card title="TX Confirm Time" sub="Aptos transaction confirmation" latest={fmtMs(avgTxConf)} color="#fb923c">
                <Chart series={[{ data: benchChron.map(h => num(h.txConfirmMs)), color: "#fb923c", name: "TX Confirm", fmt: v => fmtMs(v) }]} labels={bLabels} height={130} />
              </Card>
            </div>
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
              <div style={{ padding: "13px 18px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "var(--text-muted)" }}>{allBench.length} runs · Page {pg+1}/{Math.max(1,Math.ceil(allBench.length/PG))}</div>
                </div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  ...
                  <tbody>
                    {pagedBench.map((h, i) => {
                      ...
                      return <tr key={h.id||String(i)} style={{ borderTop:"1px solid var(--border-soft)" }}>
                        <td style={{padding:"8px 13px"}}><DeviceBadge h={h} /></td>
                        <td style={{padding:"8px 13px",fontSize:11,color:"var(--text-dim)",fontFamily:"monospace",whiteSpace:"nowrap"}}>{ts_}</td>
                        ...
                        <td style={{padding:"8px 13px",fontFamily:"monospace",color:accentColor,whiteSpace:"nowrap"}}>{fmtKbs(h.avgUploadKbs)}</td>
