"use client";
// components/world-map-inner.tsx — v5.3
// FIX: Added testnet AZ name mapping (Stakely-0, Jump-AMS-0 etc.) so markers show on map
// FIX: scroll wheel only zooms when cursor is inside the map
...
// Shelbynet standard DC zones
const DC_ZONES: Record<string, { lng: number; lat: number; label: string; short: string; flag: string }> = {
...
// Testnet provider DC names → geo coords
// From StorageProviderQueryResult.md and Source.md AZ structure
const TESTNET_DC_ZONES: Record<string, { lng: number; lat: number; label: string; short: string; flag: string }> = {
  "Jump-AMS-0": { lng:   4.90, lat:  52.37, label: "Amsterdam (Jump)",   short: "AMS", flag: "🇳🇱" },
  "Jump-AMS-1": { lng:   4.92, lat:  52.38, label: "Amsterdam (Jump)",   short: "AMS", flag: "🇳🇱" },
  "Jump-LON-0": { lng:  -0.13, lat:  51.51, label: "London (Jump)",      short: "LON", flag: "🇬🇧" },
  "Jump-LON-1": { lng:  -0.15, lat:  51.52, label: "London (Jump)",      short: "LON", flag: "🇬🇧" },
  "Stakely-0":  { lng:   8.68, lat:  50.11, label: "Frankfurt (Stakely)", short: "FRA", flag: "🇩🇪" },
  "Duoro-0":    { lng:  -9.14, lat:  38.72, label: "Lisbon (Duoro)",      short: "LIS", flag: "🇵🇹" },
  "Nova-0":     { lng:  -3.70, lat:  40.41, label: "Madrid (Nova)",       short: "MAD", flag: "🇪🇸" },
  "Republic-0": { lng: -74.01, lat:  40.71, label: "New York (Republic)", short: "NYC", flag: "🇺🇸" },
  "AR-0":       { lng: -122.42, lat: 37.77, label: "San Francisco (AR)",  short: "SFO", flag: "🇺🇸" },
  "AR-1":       { lng: -122.44, lat: 37.78, label: "San Francisco (AR)",  short: "SFO", flag: "🇺🇸" },
};

// Combined lookup: works for both shelbynet (dc_*) and testnet (named DCs)
const ALL_ZONES: Record<string, { lng: number; lat: number; label: string; short: string; flag: string }> = {
  ...DC_ZONES,
  ...TESTNET_DC_ZONES,
};
...
function GeoShare({ byZone, isDark }: { byZone: Map<string, StorageProvider[]>; isDark: boolean }) {
  ...
  const entries = Array.from(byZone.entries()).map(([z, sps], i) => ({
    zone: z, label: ALL_ZONES[z]?.label ?? z,
    flag: ALL_ZONES[z]?.flag ?? "🌐",
    count: sps.length, pct: total > 0 ? sps.length / total * 100 : 0,
    ...
    <div style={{ background: bg, border: `1px solid ${bdr}`, borderRadius: 13, padding: "12px 14px", width: 270, backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)" }}>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 10 }}>
        <svg width={96} height={96} viewBox="0 0 96 96" style={{ flexShrink: 0 }}>
          {entries.map(e => {
            const d = e.pct / 100 * circ, g = circ - d;
            const seg = <circle key={e.zone} cx={cx} cy={cy} r={R} fill="none" stroke={e.color} strokeWidth={stroke} strokeDasharray={`${d} ${g}`} strokeDashoffset={-off} transform={`rotate(-90 ${cx} ${cy})`} opacity={0.9} />;
            off += d;
            return seg;
          })}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 5, flex: 1, minWidth: 0 }}>
          ...
          {entries.length > 8 && <div style={{ fontSize: 9, color: pm }}>+{entries.length - 8} more zones</div>}
        </div>
      </div>
        ...
        {sps.map((p, i) => {
          ...
          return (
            <div key={p.address || i} style={{
              background: isH ? "rgba(34,197,94,0.07)" : "rgba(100,116,139,0.07)",
              border: `1px solid ${isH ? "rgba(34,197,94,0.2)" : "rgba(100,116,139,0.2)"}`,
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                ...
                <div style={{ display: "flex", gap: 3, flexShrink: 0, marginLeft: 6 }}>
                  <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 6px", borderRadius: 4, background: isH ? "rgba(34,197,94,0.15)" : "rgba(100,116,139,0.15)", color: isH ? "#22c55e" : "#94a3b8" }}>{p.health}</span>
              ...
              {p.geo?.city && (
                <div style={{ fontSize: 10, color: pm, marginBottom: 4 }}>{p.geo.city}{p.geo.countryCode ? `, ${p.geo.countryCode}` : ""}</div>
              )}
              {p.netAddress && (
...
export default function WorldMapInner({ providers }: { providers: StorageProvider[] }) {
  ...
  // Group providers by AZ — works for both dc_* and named AZs
  ...
  // Only render zones that have geo data in ALL_ZONES
  const availableZones = Array.from(byZone.keys()).filter(z => ALL_ZONES[z]);
  const ZONE_LIST = availableZones.map((z, i) => ({
    key: z,
    ...ALL_ZONES[z],
    color: ZONE_COLORS[i % ZONE_COLORS.length],
    sps: byZone.get(z) ?? [],
    ...
    >
      ...
      >
        ...
        {ZONE_LIST.map((zd, zi) => {
          ...
          const c       = zd.color;
          const glowC   = allOk ? c : (healthy > 0 ? "#f59e0b" : "#ef4444");
