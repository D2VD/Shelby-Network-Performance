// app/api/v1/network/stats/route.ts
// Proxies to backend GET /api/v1/network/stats?network=
// Backend returns { error: "..." } on 400/503, matching the export route's
// error-passthrough pattern — safe to reuse that shape here.
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    upstream = await fetch(`${VPS_URL}/api/v1/network/stats?${qs}`, {
      next: { revalidate: 0 },
    });
  } catch {
