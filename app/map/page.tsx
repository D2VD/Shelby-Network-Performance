...
// Defined locally: next/dynamic wraps the module and does not re-export named
// types, so `import type { GlobeHandle }` from the globe module raises ts2614.
...
// next/dynamic strips ForwardRef typing; cast explicitly so ref={globeRef}
...
const Globe = dynamic(
  ...
  { ssr: false, loading: () => <Loader label="Loading globe…" /> }
) as unknown as GlobeWithRef;
...
function providerToMarker(p: StorageProvider): GlobeMarker | null {
  ...
  // Globe v8.1 expects { address, lat, lng, health, az } — NOT the old cobe shape
  return {
    address: p.address,
    lat,
    lng,
