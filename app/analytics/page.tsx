"use client";
/**
 * app/analytics/page.tsx — v11.0
 * UNIFIED layout for Shelbynet and Testnet — same 6 stat cards, same 4 charts
 * FIXES:
 * 1. Preserves last known values when live returns 0 (no jump to 0)
 * 2. Handles 404/503 gracefully with fallback to /stats cache
 * 3. Identical layout for both networks (different data sources only)
 */
...
// Merge two snaps: keep higher/non-zero values to prevent zeroing out
function mergeSnap(prev: LiveSnap | null, next: LiveSnap): LiveSnap {
  if (!prev) return next;
  const keep = <T extends number>(a: T, b: T) => (b > 0 ? b : a);
  return {
    ...next,
    activeBlobs:       keep(prev.activeBlobs,       next.activeBlobs),
    totalBlobEvents:   keep(prev.totalBlobEvents,   next.totalBlobEvents),
    totalStorageBytes: keep(prev.totalStorageBytes, next.totalStorageBytes),
    totalStorageGB:    keep(prev.totalStorageGB,    next.totalStorageGB),
    totalStorageGiB:   keep(prev.totalStorageGiB,   next.totalStorageGiB),
    storageProviders:  keep(prev.storageProviders,  next.storageProviders),
    placementGroups:   keep(prev.placementGroups,   next.placementGroups),
    slices:            keep(prev.slices,            next.slices),
  };
}
  ...
  return (
    <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 14, padding: "16px 20px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12, flexWrap: "wrap", gap: 6 }}>
        ...
        <div style={{ fontFamily: "monospace", fontSize: 16, fontWeight: 700, color: latestColor, wordBreak: "break-all" }}>{latest}</div>
      </div>
...
export default function AnalyticsPage() {
  ...
  const [lastAtStr, setLastAtStr] = useState<string>("");  // client-only, avoids hydration mismatch
  const [error,     setError]     = useState<string | null>(null);
  const [isStale,   setIsStale]   = useState(false);

  useEffect(() => { alive.current = true; return () => { alive.current = false; }; }, []);

  const applyData = useCallback((data: LiveSnap) => {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      if (res.status !== 404 && res.status !== 503) {
        ...
        if (data && (data.blockHeight != null || data.storageProviders != null)) {
          ...
          if (alive.current) setIsStale(method.includes("stale") || method.includes("cache") || method.includes("fallback"));
          if (alive.current) setError(null);
          applyData(data as LiveSnap);
    ...
    try {
      const r2 = await fetch(`/api/network/stats?network=${network}`, { signal: AbortSignal.timeout(12_000) });
      if (r2.ok) {
  ...
  useEffect(() => {
    if (alive.current) { setLoading(true); setSeries([]); setError(null); setIsStale(false); }
    fetchStats();
    const id = setInterval(fetchStats, POLL_MS);
    return () => clearInterval(id);
  }, [fetchStats]);

  // ── Metric definitions — same 6 cards for both networks ───────────────────
  const METRICS = isTestnet ? [
    ...
    { label: "Waitlisted SPs",    value: fmt(snap?.waitlistedProviders), sub: "In waitlist queue",        icon: "◎", color: "#f59e0b" },
  ...
  // ── Chart definitions — same 4 charts for both networks ──────────────────
  ...
  return (
    <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 4px" }}>
      <style>{`
        @media(max-width:768px){.ag2{grid-template-columns:1fr!important}.ag3{grid-template-columns:1fr 1fr!important}.ag4{grid-template-columns:1fr!important}}
        @media(max-width:480px){.ag3{grid-template-columns:1fr!important}}
      `}</style>
      ...
      {isTestnet && (
        <div style={{ background: "rgba(147,51,234,0.07)", border: "1px solid rgba(147,51,234,0.25)", borderRadius: 10, padding: "10px 16px", marginBottom: 16, fontSize: 13, color: "#c084fc", display: "flex", alignItems: "center", gap: 8 }}>
          <span>⚗</span>
          <span>Shelby Testnet · Contract: 0x85fdb9a1… · Chain ID: {snap?.chainId ?? 2}</span>
        </div>
      )}
      ...
      {/* Block + Breakdown — same for both networks */}
