/**
 * workers/geo-sync.ts — v4.2 FINAL
 *
 * KẾT LUẬN SAU TOÀN BỘ RESEARCH:
 * - Không có on-chain aggregate cho totalStorageUsed
 * - Explorer iterate toàn bộ blobs để sum → ta không replicate được trong timeout
 * - account_transactions là nguồn duy nhất ổn định cho tất cả metrics
 *
 * CÔNG THỨC CUỐI CÙNG (validated từ dữ liệu thực):
 *
 * totalBlobs:
 *   = binary search trên current_table_items (exact, ~10s)
 *   Confirmed: blob table rows = 1:1 với blobs
 *
 * totalBlobEvents:
 *   = account_transactions_count × 2.0
 *   Basis: mỗi blob = register_multiple_blobs + add_blob_acknowledgements = 2 txns
 *   Ratio confirmed: 562212 txns → 1126332 events (×2.0034)
 *
 * totalStorageUsedBytes:
 *   = totalBlobs × AVG_BLOB_SIZE_BYTES
 *   Basis: calibrated từ Explorer: 169.92 GB / 563,894 blobs = 301,333 bytes/blob
 *   Note: ±5% variance do blob size distribution thay đổi theo thời gian
 *   Đây là số gần đúng tốt nhất có thể mà không iterate toàn bộ
 *
 * totalBlobs vs account_txns:
 *   txns/blobs = 0.9906 → txns ≈ blobs (rất gần)
 *   Có thể dùng txns × (1/0.9906) = txns × 1.0095 ≈ blobs nếu cần fast estimate
 *
 * storageProviders/placementGroups/slices:
 *   = on-chain RPC (exact, instant)
 */
...
const NETWORKS = {
  shelbynet: {
    ...
    // Calibrated: 169.92 GB / 563,894 blobs = 301,333 bytes/blob avg
    // Using a slightly more conservative estimate to account for growth trend
    avgBlobSizeBytes: 301_333,
    // Events/txns ratio: 1,126,332 / 562,212 = 2.0034
    eventsPerTxn: 2.0,
  },
  testnet: {
...
// ── 3. Storage + Providers/PGs/Slices ────────────────────────────────────────
...
async function fetchBlobStats(network: NetworkId): Promise<KVStats> {
  ...
  const avgBlobSize  = (cfg as any).avgBlobSizeBytes as number;
  const eventsPerTxn = (cfg as any).eventsPerTxn as number;

  return {
    totalBlobs,
    // storage = blobs × calibrated_avg_size (±5%)
    totalStorageUsedBytes: Math.round(totalBlobs * avgBlobSize),
    // events = txns × 2.0 (very stable ratio)
    totalBlobEvents:       txnCount > 0 ? Math.round(txnCount * eventsPerTxn) : 0,
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/health")    return Response.json({ ok: true, worker: "shelby-geo-sync", version: "4.2.0", ts: new Date().toISOString() }, { headers: CORS });
