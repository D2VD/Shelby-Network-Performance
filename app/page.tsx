...
// ── Diagnose panel (Đã fix lỗi undefined) ─────────────────────────────────────
        ...
        <div>
          ...
          <div className="card-subtitle">
            {loading ? "Checking…" : result
              ? result.ready
                ? `All ${result.passCount} checks passed${result.workerVersion ? ` · API v${result.workerVersion}` : ""}`
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6, paddingTop: 16 }}>
            {result.checks.map((c, i) => (
              <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "8px 10px", borderRadius: 8, background: c.status === "fail" ? "#fef2f2" : c.status === "warn" ? "#fffbeb" : "#f9fafb" }}>
                <span style={{ fontFamily: "var(--font-mono)", fontSize: 13, fontWeight: 700, color: statusColor[c.status], flexShrink: 0, width: 16, paddingTop: 2 }}>
                  {statusIcon[c.status]}
                </span>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 16, fontSize: 11, color: "var(--gray-400)", marginTop: 4 }}>
        ...
        <span style={{ marginLeft: "auto" }}>{recent.length} runs</span>
      </div>
    </div>
  );
}
  ...
  const runDiagnose = useCallback(async () => {
    ...
    try {
      ...
      setDiagnose({
        ...
        checks:[{ name: "API Connection", status: "fail", hint: e.message }],
        summary: "Cannot connect to Backend API",
      });
    ...
    try {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        try {
          ...
          uploads.push(u);
          addLog(`  ✓ ${label}: ${fmt(u.speedKbs)} · ${fmtMs(u.elapsed)}`);
        } catch (e: any) {
      ...
      call("/api/benchmark/results", res).then(() => loadHistory()).catch(() => {});
    ...
    <div>
      <div className="page-header">
        ...
        <p className="page-subtitle">
          Measure upload speed, download speed, latency, and transaction time on <strong>{config.label}</strong>
        </p>
      </div>
        ...
        <div className="card">
          <div className="card-header">
            <div>
              ...
              <div className="card-subtitle">{history.length} benchmark runs</div>
            </div>
          </div>
