/**
 * workers/geo-sync.ts — v2.2
 *
 * FIX: /stats endpoint đọc từ R2 snapshot thay vì gọi Explorer API
 * R2 snapshot được CRON cập nhật mỗi giờ với số liệu chính xác từ Explorer
 * → Không bị block, luôn có data đúng
 *
 * CRON flow: mỗi giờ → fetchNetworkStats (gọi Explorer) → ghi R2
 * /stats flow: đọc R2 snapshot gần nhất → trả về ngay
 */
...
// Fetch stats: try Explorer first, fallback on-chain
...
async function syncNetwork(network: NetworkId, env: Env, ctx: ExecutionContext): Promise<{ synced: number; errors: string[] }> {
  ...
  // Write stats snapshot to R2 (CRON only — gọi Explorer từ đây)
...
async function handleNodes(url: URL, env: Env): Promise<Response> {
  ...
  try {
    ...
    return Response.json({ ok: false, error: e.message, data: { providers: [], count: 0 } }, { status: 500, headers: CORS });
  }
}
...
async function handleSnapshots(url: URL, env: Env): Promise<Response> {
  ...
  try {
    ...
    const snapshots = (await Promise.all(keys.map(async key => { const obj = await env.SHELBY_R2.get(key); if (!obj) return null; return JSON.parse(await obj.text()) as StatsSnapshot; }))).filter(Boolean).sort((a, b) => new Date(a!.ts).getTime() - new Date(b!.ts).getTime()) as StatsSnapshot[];
    return Response.json({ ok: true, network, data: { snapshots, count: snapshots.length }, fetchedAt: new Date().toISOString() }, { headers: { ...CORS, "Cache-Control": "public, max-age=300" } });
  } catch (e: any) {
    return Response.json({ ok: false, error: e.message, data: { snapshots: [], count: 0 } }, { status: 500, headers: CORS });
  }
}

// ── /stats: Read from R2 snapshot (correct data, no Explorer block issue) ──────
async function handleStats(url: URL, env: Env): Promise<Response> {
  ...
  // 1. Node info (luôn fresh)
  let node = null;
  try {
    const r = await fetch(`${cfg.nodeUrl}/`, { signal: AbortSignal.timeout(5000) });
    if (r.ok) {
      const d: any = await r.json();
      node = { blockHeight: nb(d.block_height), ledgerVersion: nb(d.ledger_version), chainId: nb(d.chain_id) };
    }
  } catch {}

  // 2. Stats từ R2 snapshot gần nhất (được CRON ghi mỗi giờ với data từ Explorer)
  ...
  try {
    ...
    console.error("[stats] R2 read failed:", e.message);
  }

  // 3. Fallback: on-chain RPC nếu R2 không có data
  if (source === "none") {
    try {
      ...
      if (slR.status === "fulfilled" && slR.value.ok) { const d: any = await slR.value.json(); const sl = nb(d?.data?.slices?.big_vec?.vec?.[0]?.end_index) + nb(d?.data?.slices?.inline_vec?.length); stats.slices = sl; stats.totalBlobs = sl > 0 ? Math.ceil(sl / 16) : 0; }
      source = "on-chain";
    } catch {}
  }
  ...
  return Response.json({
    ...
    data: {
      node, stats, network,
      statsSource: `worker-${source}`,
      snapshotTs: snapshotAge || null,
    },
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/health")    return Response.json({ ok: true, worker: "shelby-geo-sync", version: "2.2.0", ts: new Date().toISOString() }, { headers: CORS });
