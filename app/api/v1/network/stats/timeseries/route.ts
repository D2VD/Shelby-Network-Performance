// app/api/v1/network/stats/timeseries/route.ts
// Proxies to backend GET /api/v1/network/stats/timeseries?network=&resolution=&range=
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    upstream = await fetch(`${VPS_URL}/api/v1/network/stats/timeseries?${qs}`, {
      next: { revalidate: 0 },
    });
  } catch {
