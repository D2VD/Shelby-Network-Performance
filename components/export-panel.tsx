// components/export-panel.tsx — v1.0
// Priority 3: Data Export API — frontend download panel
// Renders inside the Explorer tab (or any settings panel).
// Calls /api/v1/export/* on the VPS directly (not proxied — binary response).
//
// Usage:
//   <ExportPanel network="shelbynet" />
...
type ExportTarget  = "snapshots" | "sp-history";

interface DownloadState {
  loading: boolean;
  error:   string | null;
  lastOk:  string | null; // ISO timestamp of last successful download
}
...
async function triggerDownload(url: string, filename: string): Promise<void> {
  ...
  if (res.status === 429) throw new Error("Rate limit: 1 export per hour per endpoint.");
...
function SnapshotsPanel({
  ...
  const handleExport = async () => {
    ...
    try {
      const url = `${VPS_BASE}/api/v1/export/snapshots?network=${encodeURIComponent(network)}&from=${from}T00:00:00Z&to=${to}T23:59:59Z&format=${fmt}`;
      const filename = `shelby-snapshots-${network}-${from}-${to}.${fmt}`;
...
// ── Sub-panel: SP History ─────────────────────────────────────────────────────

function SPHistoryPanel({
  network,
  isDark,
}: {
  network: string;
  ...
  const handleExport = async () => {
    ...
    setState({ loading: true, error: null, lastOk: null });
    try {
      const url = `${VPS_BASE}/api/v1/export/sp-history?network=${encodeURIComponent(network)}&address=${encodeURIComponent(address.trim())}&from=${from}T00:00:00Z&to=${to}T23:59:59Z&format=${fmt}`;
      const shortAddr  = address.trim().slice(0, 8);
      const filename   = `sp-history-${shortAddr}-${from}-${to}.${fmt}`;
      await triggerDownload(url, filename);
      setState({ loading: false, error: null, lastOk: new Date().toISOString() });
    } catch (err) {
      setState({ loading: false, error: (err as Error).message, lastOk: null });
  ...
  return (
    <ExportCard
      ...
      extraFields={
        <div>
          <label className={`block text-xs mb-1 ${isDark ? "text-white/50" : "text-black/50"}`}>
            SP Address
          </label>
          <input
            className={inputCls}
...
function ExportCard({
  ...
  const inputCls  = isDark
    ? "rounded-lg border border-white/15 bg-white/5 text-white/80 text-xs px-3 py-2 focus:outline-none focus:border-white/30"
    : "rounded-lg border border-black/15 bg-black/5 text-black/80 text-xs px-3 py-2 focus:outline-none focus:border-black/30";
  const btnCls    = isDark
    ? "flex items-center gap-2 px-4 py-2 rounded-lg bg-white/10 hover:bg-white/15 text-white/80 text-xs font-semibold transition-colors disabled:opacity-40"
    : "flex items-center gap-2 px-4 py-2 rounded-lg bg-black/8 hover:bg-black/12 text-black/70 text-xs font-semibold transition-colors disabled:opacity-40";
      ...
      <div className="space-y-3">
            ...
            <div className="flex">
              {(["csv", "json"] as ExportFormat[]).map((f) => (
                <button
                  key={f}
                  onClick={() => setFmt(f)}
                  className={`px-3 py-1.5 text-xs font-mono border first:rounded-l-lg last:rounded-r-lg transition-colors ${
                    fmt === f ? fmtActive : fmtInact
                  }`}
                >
                  {f.toUpperCase()}
                </button>
        ...
        {state.lastOk && !state.error && (
          <p className={`text-xs mt-1 ${isDark ? "text-green-400/70" : "text-green-600/70"}`}>
            Downloaded at {new Date(state.lastOk).toLocaleTimeString("en-US", { timeZone: "UTC", hour12: false })} UTC
          </p>
        )}
      </div>
    </div>
