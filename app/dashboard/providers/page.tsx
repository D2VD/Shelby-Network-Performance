"use client";
/**
 * app/dashboard/providers/page.tsx — v9.0
 * Fix: "16 nodes online" badge moved to right side → không đè lên GeoShare (left)
 * GeoShare panel chiếm ~285px bên trái → badges đặt bên phải
 */
...
function BlsKey({full}:{full:string}) {
  ...
  const copy=async(e:React.MouseEvent)=>{e.stopPropagation();await navigator.clipboard.writeText(full).catch(()=>{});setCopied(true);setTimeout(()=>setCopied(false),1500);};
...
// Fixed right-side tooltip
function HoverTooltip({p}:{p:StorageProvider}) {
  const {isDark}=useTheme();
  const [copied,setCopied]=useState(false);
  const bls=p.fullBlsKey||p.blsKey||"";
  const isH=p.health==="Healthy";
  const copy=async()=>{if(!bls)return;await navigator.clipboard.writeText(bls).catch(()=>{});setCopied(true);setTimeout(()=>setCopied(false),1500);};
  ...
  return(
    <div style={{position:"fixed",right:24,top:"50%",transform:"translateY(-50%)",zIndex:600,width:288,
      background:bg,border:`1px solid ${bdr}`,borderRadius:14,padding:"15px 17px",
      backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",
      boxShadow:"0 20px 50px rgba(0,0,0,0.25)",pointerEvents:"none"}}>
      ...
      {bls&&(
        <div style={{borderTop:`1px solid ${bdr}`,paddingTop:10}}>
          ...
          <div style={{fontSize:9,color:pm,marginTop:3,opacity:.7}}>on-chain · storage_provider_registry</div>
        </div>
      )}
    </div>
  );
...
export default function ProvidersPage() {
  ...
  useEffect(()=>{setLoading(true);setProviders([]);fetchProviders();const id=setInterval(fetchProviders,60_000);return()=>clearInterval(id);},[fetchProviders]);
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:0,minHeight:"calc(100vh - 120px)",background:"var(--bg-primary)"}}>
      ...
      <div style={{background:isDark?"#0d1526":"#f0f4f8",position:"relative",height:"55vh",minHeight:340}}>
        ...
        {/* FIX: Badges moved to TOP RIGHT — không đè lên GeoShare (top left) */}
