"use client";
/**
 * app/benchmark/page.tsx — Benchmark Page v5.7
 * FIX: Download sends expectedBytes so backend can fall back to upload size
 * All other logic from v5.6 preserved
 */
...
// NOTE: this file (benchmark-page-client.tsx) is loaded ONLY via next/dynamic
// with ssr:false from app/benchmark/page.tsx. Every import in this file is
// therefore isolated inside that dynamic chunk and never reachable from
// page.tsx's static/server-resolved module graph — that isolation is what
// keeps @aptos-labs/wallet-adapter-react's dependency tree (which pulls in
// `got` and @telegram-apps/bridge, neither resolvable in this project's
// Cloudflare Pages build) out of the build entirely. Do NOT import anything
// from this file directly into page.tsx or any other statically-imported
// module — always go through the dynamic() wrapper.
...
interface DiagnoseResult { ready:boolean; passCount:number; failCount:number; warnCount:number; checks:DiagnoseCheck[]; summary:string; workerVersion?:string }
interface BenchResult { latency:LatResult; uploads:UpResult[]; downloads:DlResult[]; tx:TxResult; avgUploadKbs:number; avgDownloadKbs:number; score:number; tier:string; runAt:string; maxSuccessfulBytes?:number; mode:"adaptive"|"standard" }
...
const fmtBytes = (b:number) => { if(b>=1_048_576)return`${(b/1_048_576).toFixed(2)} MB`; if(b>=1024)return`${(b/1024).toFixed(1)} KB`; return`${b} B`; };

function calcScore(r:{avgUploadKbs:number;avgDownloadKbs:number;latency:LatResult;tx:TxResult}) {
  const up=Math.min(400,r.avgUploadKbs*1.0), dl=Math.min(250,r.avgDownloadKbs*0.05);
  const lat=200*Math.max(0,1-r.latency.avg/5000), tx=150*Math.max(0,1-r.tx.confirmTime/8000);
  const score=Math.round(up+dl+lat+tx);
  let tier="Poor";
  if(score>=900)tier="Blazing Fast"; else if(score>=700)tier="Excellent"; else if(score>=450)tier="Good"; else if(score>=200)tier="Fair";
  return {score,tier};
}
...
const callWithDevice = async <T,>(url:string, body?:object): Promise<T> => {
  ...
  const r = await fetch(url, {
    ...
    headers: { ...(body?{"Content-Type":"application/json"}:{}), "x-device-id":deviceId },
    body: body?JSON.stringify(body):undefined,
  });
  if (!r.ok) { const err:any=await r.json().catch(()=>({error:`HTTP ${r.status}`})); throw new Error(err.error||`HTTP ${r.status}`); }
  return r.json();
};
...
function LogLine({text}:{text:string}) {
  const isErr=text.startsWith("✗"),isOk=text.startsWith("✓")||text.startsWith("Done"),isHdr=text.startsWith("—"),isSub=text.startsWith("  ");
  return <div style={{fontFamily:"var(--font-mono)",fontSize:13,lineHeight:1.75,paddingLeft:isSub&&!isOk?14:0,color:isErr?"#ef4444":isOk?"#16a34a":isHdr?"#6366f1":isSub?"#9ca3af":"#c0ccd8"}}>{text}</div>;
}
    ...
    <div>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:8,fontSize:12,fontFamily:"var(--font-mono)",color:"var(--gray-400)"}}>
        <span style={{color:running?"var(--gray-700)":"var(--gray-400)",fontWeight:running?600:400}}>
          {running?STEPS.find(s=>s.phase===phase)?.label??phase:phase==="done"?"Complete":phase==="error"?"Error":"Ready"}
        </span>
        <span>{pct}%</span>
      </div>
      <div style={{height:5,background:"var(--gray-100)",borderRadius:10,overflow:"hidden"}}>
        <div style={{height:"100%",width:`${pct}%`,borderRadius:10,transition:"width 0.6s ease,background 0.3s",background:phase==="error"?"#ef4444":phase==="done"?"#16a34a":"var(--net-color,#2563eb)",boxShadow:running?"0 0 8px var(--net-color,#2563eb)88":"none"}}/>
      </div>
      <div style={{display:"flex",justifyContent:"space-between",marginTop:8}}>
        {STEPS.map(s=>{
          const done=PHASE_PCT[phase]>=s.pct&&phase!=="idle",current=s.phase===phase;
          return <div key={s.phase} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,flex:1,opacity:done||current?1:0.3}}>
            <div style={{width:20,height:20,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,background:done?(phase==="error"?"#ef4444":"var(--net-color,#2563eb)"):current?"transparent":"#f0f0f0",border:current?"2px solid var(--net-color,#2563eb)":"2px solid transparent",color:done?"#fff":current?"var(--net-color,#2563eb)":"#999",animation:current&&running?"pulse-ring 1.5s infinite":"none"}}>
              {done?"✓":s.icon}
            </div>
            <span style={{fontSize:8,fontFamily:"var(--font-mono)",color:current?"var(--net-color)":"var(--gray-400)",textAlign:"center"}}>{s.label}</span>
          </div>;
        })}
      </div>
    </div>
...
function ScoreRing({score,tier}:{score:number;tier:string}) {
  const color=TIER_COLOR[tier]??"#6b7280",r=44,c=2*Math.PI*r,dash=(Math.min(100,(score/1000)*100)/100)*c;
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
      <svg width={110} height={110} viewBox="0 0 110 110">
        <circle cx={55} cy={55} r={r} fill="none" stroke="#f0f0f0" strokeWidth={8}/>
        <circle cx={55} cy={55} r={r} fill="none" stroke={color} strokeWidth={8} strokeDasharray={`${dash} ${c}`} strokeLinecap="round" transform="rotate(-90 55 55)" style={{transition:"stroke-dasharray 1.2s cubic-bezier(.4,0,.2,1)"}}/>
...
function TxHashCell({hash,network}:{hash:string|null;network:string}) {
  ...
  return <span style={{display:"inline-flex",alignItems:"center",gap:5}}>
    <a href={`https://explorer.aptoslabs.com/txn/${hash}?network=${network}`} target="_blank" rel="noreferrer" style={{color:"var(--info,#2563eb)",fontSize:12,fontFamily:"var(--font-mono)",textDecoration:"none"}} title={hash}>{hash.slice(0,8)}…{hash.slice(-5)} ↗</a>
    <button onClick={e=>{e.preventDefault();navigator.clipboard.writeText(hash).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),1500)});}} style={{background:"none",border:"none",cursor:"pointer",padding:"0 2px",fontSize:11,color:copied?"#16a34a":"var(--gray-400)"}}>{copied?"✓":"⎘"}</button>
  </span>;
}
...
function DiagnosePanel({result,loading,onRecheck}:{result:DiagnoseResult|null;loading:boolean;onRecheck:()=>void}) {
  ...
  useEffect(()=>{ if(result?.ready)setExpanded(false); if(!result?.ready&&result!==null)setExpanded(true); },[result?.ready]);
  return (
    <div className="card" style={{marginBottom:16}}>
      <div className="card-header" style={{gap:10}}>
        <div style={{display:"flex",alignItems:"center",gap:10,flex:1,minWidth:0,flexWrap:"wrap"}}>
          {!loading&&result&&<span style={{fontSize:14,fontWeight:700,color:result.ready?"#16a34a":"#ef4444"}}>{result.ready?"✓":"✗"}</span>}
          <div style={{minWidth:0}}>
            <div className="card-title">Pre-flight checks</div>
            <div className="card-subtitle">{loading?"Checking…":result?result.ready?`${result.passCount} checks passed · API v${result.workerVersion??"?"}`:`${result.failCount} issue(s) — fix before running`:"Not checked yet"}</div>
          </div>
        </div>
        <div style={{display:"flex",gap:6,flexShrink:0}}>
          {result&&!loading&&<button onClick={()=>setExpanded(v=>!v)} className="btn btn-secondary" style={{fontSize:11}}>{expanded?"▲ Hide":"▼ Details"}</button>}
          <button onClick={onRecheck} disabled={loading} className="btn btn-secondary" style={{fontSize:12}}>{loading?"⟳":"⟳ Recheck"}</button>
        </div>
      </div>
...
function CustomUploadCard({running,network}:{running:boolean;network:string}) {
  ...
  const handleUpload=async()=>{
    ...
    try {
      ...
      const r=await callWithDevice<UpResult>("/api/benchmark/upload/custom",{customData:Array.from(new Uint8Array(buf)),blobName:`custom/${file.name.replace(/[^a-zA-Z0-9._-]/g,"_")}/${Date.now()}`});
      setLastResult(r);
  ...
  return (
    <div className="card" style={{marginBottom:16}}>
      <div className="card-header"><div><div className="card-title">Custom upload</div><div className="card-subtitle">Upload your own file · max {fmtBytes(MAX_CUSTOM_BYTES)} · not rate limited</div></div></div>
      <div className="card-body" style={{display:"flex",flexDirection:"column",gap:12}}>
        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
          <div onClick={()=>inputRef.current?.click()} style={{flex:1,minWidth:160,border:"1.5px dashed var(--gray-200)",borderRadius:10,padding:"12px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:10,background:file?"#f0fdf4":"#fafafa"}}
            onMouseEnter={e=>(e.currentTarget.style.borderColor="var(--net-color,#2563eb)")}
            onMouseLeave={e=>(e.currentTarget.style.borderColor="var(--gray-200)")}>
            <span style={{fontSize:20}}>{file?"📄":"📂"}</span>
            <div style={{flex:1,minWidth:0}}>
              {file?<><div style={{fontSize:13,fontWeight:600,color:"var(--gray-800)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{file.name}</div><div style={{fontSize:11,color:"var(--gray-400)",fontFamily:"var(--font-mono)"}}>{fmtBytes(file.size)}</div></>:<div style={{fontSize:13,color:"var(--gray-400)"}}>Click to select a file</div>}
            </div>
            <input ref={inputRef} type="file" onChange={handleFile} style={{display:"none"}}/>
          </div>
          <button onClick={handleUpload} disabled={!file||uploading||running} className="btn btn-primary" style={{flexShrink:0,opacity:(!file||uploading||running)?0.5:1}}>{uploading?"Uploading…":"↑ Upload"}</button>
        </div>
        ...
        {lastResult&&(
          <div style={{padding:"10px 14px",borderRadius:8,background:"#f0fdf4",border:"1px solid #bbf7d0"}}>
            <div style={{display:"flex",gap:20,flexWrap:"wrap",alignItems:"center"}}>
              {[{label:"Speed",value:fmtKbs(lastResult.speedKbs),color:"#16a34a"},{label:"Time",value:fmtMs(lastResult.elapsed),color:"var(--gray-700)"},{label:"Size",value:fmtBytes(lastResult.bytes),color:"var(--gray-700)"}].map(({label,value,color})=>(
                <div key={label}><div style={{fontSize:10,color:"var(--gray-400)",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:2}}>{label}</div><div style={{fontFamily:"var(--font-mono)",fontSize:14,fontWeight:600,color}}>{value}</div></div>
              ))}
              <div style={{flex:1,minWidth:120}}><div style={{fontSize:10,color:"var(--gray-400)",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:2}}>Tx hash</div><TxHashCell hash={lastResult.txHash} network={network}/></div>
            </div>
          </div>
        )}
      </div>
...
function HistoryTable({displayHistory,totalCount}:{displayHistory:any[];totalCount:number}) {
  ...
  if(totalCount===0)return(
    <div className="card" style={{padding:"40px 24px",textAlign:"center"}}>
      ...
      <div style={{fontSize:12,color:"var(--gray-300)",marginTop:6}}>History is stored per browser. Other browsers start fresh.</div>
    </div>
  );
  return (
    <div className="card">
      <div className="card-header">
        <div><div className="card-title">Benchmark history</div><div className="card-subtitle">{totalCount} runs · stored in this browser{pages>1?` · Page ${page+1}/${pages}`:""}</div></div>
      </div>
      <div className="card-body" style={{padding:0,overflowX:"auto"}}>
        <table className="data-table" style={{width:"100%",minWidth:600}}>
          <thead><tr><th>#</th><th>Mode</th><th>Score</th><th>Tier</th><th>Upload</th><th>Download</th><th>Latency</th><th>TX</th><th>Max blob</th><th>At</th></tr></thead>
          <tbody>
            {paged.map((h:any)=>{
      ...
      {pages>1&&(
        <div style={{padding:"12px 20px",borderTop:"1px solid var(--gray-100)",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
          <button onClick={()=>setPage(p=>Math.max(0,p-1))} disabled={page===0} style={{padding:"5px 12px",borderRadius:7,border:"1px solid var(--gray-200)",background:"#fff",color:"var(--gray-500)",cursor:page===0?"not-allowed":"pointer",opacity:page===0?.4:1,fontSize:13}}>←</button>
          {Array.from({length:Math.min(pages,10)},(_,i)=>i).map(i=>(
            <button key={i} onClick={()=>setPage(i)} style={{padding:"5px 10px",borderRadius:7,border:"1px solid var(--gray-200)",background:i===page?"var(--net-color,#2563eb)":"#fff",color:i===page?"#fff":"var(--gray-500)",cursor:"pointer",fontWeight:i===page?700:400,fontSize:13,minWidth:34}}>{i+1}</button>
          ))}
          {pages>10&&<span style={{fontSize:12,color:"var(--gray-400)"}}>…{pages}</span>}
          <button onClick={()=>setPage(p=>Math.min(pages-1,p+1))} disabled={page===pages-1} style={{padding:"5px 12px",borderRadius:7,border:"1px solid var(--gray-200)",background:"#fff",color:"var(--gray-500)",cursor:page===pages-1?"not-allowed":"pointer",opacity:page===pages-1?.4:1,fontSize:13}}>→</button>
        </div>
      )}
    </div>
  );
...
function BenchmarkPageInner() {
  ...
  const runDiagnose=useCallback(async()=>{
    setDiagLoad(true);
    try { const d=await callWithDevice<DiagnoseResult>("/api/benchmark/diagnose"); setDiagnose(d); }
    catch(e:any) { setDiagnose({ready:false,passCount:0,failCount:1,warnCount:0,checks:[{name:"Backend API",status:"fail",hint:e.message}],summary:"Cannot connect"}); }
  ...
  const run=useCallback(async()=>{
    ...
    try {
      ...
      setBalance(bal); if(myRun!==runIdRef.current)return;
      ...
      if(benchMode==="adaptive"){
        ...
        for(let i=0;i<ADAPTIVE_SIZES.length;i++){
          ...
          try {
            ...
            if(myRun!==runIdRef.current)return;
            uploads.push(u); maxSuccessfulBytes=bytes;
            addLog(`  ✓ ${fmtBytes(bytes)}: ${fmtKbs(u.speedKbs)} · ${fmtMs(u.elapsed)}${u.txHash?` · tx ${u.txHash.slice(0,8)}…`:""}`);
          } catch(e:any) { addLog(`  ✗ ${fmtBytes(bytes)}: ${e.message} — stopping`); break; }
        }
        ...
        for(let i=0;i<STANDARD_SIZES.length;i++){
          ...
          if(myRun!==runIdRef.current)return;
          uploads.push(u); maxSuccessfulBytes=sz;
      ...
      for(const up of blobsToDownload){
        if(myRun!==runIdRef.current)return;
        try {
          // FIX: send expectedBytes so backend can fall back to upload size
          const d=await callWithDevice<DlResult>("/api/benchmark/download",{
            blobName:up.blobName,
            expectedBytes:up.bytes,
          });
          if(myRun!==runIdRef.current)return;
          downloads.push(d);
  ...
  const requestFaucet=useCallback(async()=>{
    ...
    try{await callWithDevice("/api/benchmark/faucet",{});addLog("✓ Requested — balance updates in ~5s");setTimeout(refreshBalance,5000);}
  ...
  return (
    <div>
      <style>{`
        @keyframes pulse-ring { 0%{box-shadow:0 0 0 0 var(--net-color,#2563eb)55} 70%{box-shadow:0 0 0 7px transparent} 100%{box-shadow:0 0 0 0 transparent} }
      ...
      {activeTab==="run"&&(
        <>
          <div className="card" style={{marginBottom:16,padding:"18px 20px"}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,flexWrap:"wrap"}}>
              ...
              {(["adaptive","standard"] as const).map(m=>(
                <button key={m} onClick={()=>setBenchMode(m)} disabled={running} style={{padding:"5px 12px",borderRadius:8,fontSize:12,fontWeight:benchMode===m?600:400,cursor:"pointer",border:`1px solid ${benchMode===m?"var(--net-color,#2563eb)":"var(--gray-200)"}`,background:benchMode===m?"var(--net-bg,#eff6ff)":"transparent",color:benchMode===m?"var(--net-color,#2563eb)":"var(--gray-500)"}}>
          ...
          <div style={{marginBottom:12}}><button onClick={()=>setShowCustom(v=>!v)} className="btn btn-secondary" style={{fontSize:12}}>{showCustom?"▲ Hide":"▼"} Custom upload (max {fmtBytes(MAX_CUSTOM_BYTES)})</button></div>
          {showCustom&&<CustomUploadCard running={running} network={network}/>}

          {log.length>0&&(
            <div className="card" style={{marginBottom:16}}>
              <div className="card-header"><div className="card-title">System log</div><button onClick={()=>setLog([])} className="btn btn-secondary" style={{fontSize:11}}>Clear</button></div>
              <div ref={logRef} className="bench-log" style={{maxHeight:240,overflowY:"auto"}}>{log.map((line,i)=><LogLine key={i} text={line}/>)}</div>
            </div>
          )}

          {result&&phase==="done"&&(
            <div className="card" style={{marginBottom:16}}>
              <div className="card-header"><div><div className="card-title">Benchmark results</div><div className="card-subtitle">{config.label} · {result.runAt} · {result.mode}</div></div></div>
              <div className="card-body">
                <div style={{display:"flex",gap:24,alignItems:"flex-start",flexWrap:"wrap",marginBottom:24}}>
                  ...
                  <div style={{flex:1,minWidth:200}}>
                    ...
                    <div style={{display:"flex",gap:18,marginTop:14,flexWrap:"wrap"}}>
                      {[{label:"Avg latency",value:fmtMs(result.latency.avg)},{label:"TX submit",value:fmtMs(result.tx.submitTime)},{label:"TX confirm",value:fmtMs(result.tx.confirmTime)}].map(({label,value})=>(
                        <div key={label}><div style={{fontSize:10,color:"var(--gray-400)",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:3}}>{label}</div><div style={{fontFamily:"var(--font-mono)",fontSize:15,fontWeight:600,color:"var(--gray-800)"}}>{value}</div></div>
                      ))}
                    </div>
                  </div>
                </div>
                <div style={{padding:"12px 14px",borderRadius:8,background:"#f9fafb",border:"1px solid #f0f0f0",marginBottom:18}}>
                  ...
                  <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
                    {[{label:"Upload 40%",pts:Math.min(400,result.avgUploadKbs*1.0),max:400,color:"#2563eb"},{label:"Download 25%",pts:Math.min(250,result.avgDownloadKbs*0.05),max:250,color:"#16a34a"},{label:"Latency 20%",pts:Math.round(200*Math.max(0,1-result.latency.avg/5000)),max:200,color:"#9333ea"},{label:"TX 15%",pts:Math.round(150*Math.max(0,1-result.tx.confirmTime/8000)),max:150,color:"#f59e0b"}].map(({label,pts,max,color})=>(
                      <div key={label} style={{flex:1,minWidth:100}}>
                        <div style={{display:"flex",justifyContent:"space-between",marginBottom:3,fontSize:10}}><span style={{color:"var(--gray-500)"}}>{label}</span><span style={{fontFamily:"var(--font-mono)",color,fontWeight:600}}>{Math.round(pts)}/{max}</span></div>
                        <div style={{height:4,background:"#f0f0f0",borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:`${(pts/max)*100}%`,background:color,borderRadius:2,transition:"width 1s"}}/></div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div style={{overflowX:"auto"}}>
                  <table className="data-table" style={{width:"100%",minWidth:400}}>
                    ...
                    <tbody>{result.uploads.map((u,i)=><tr key={i}><td><span className="mono">{fmtBytes(u.bytes)}</span></td><td><span className="mono" style={{color:"#2563eb",fontWeight:600}}>{fmtKbs(u.speedKbs)}</span></td><td><span className="mono" style={{color:"var(--gray-500)"}}>{fmtMs(u.elapsed)}</span></td><td><TxHashCell hash={u.txHash} network={network}/></td></tr>)}</tbody>
                  </table>
                </div>
                  ...
                  <>
                    ...
                    <div style={{overflowX:"auto"}}>
                      <table className="data-table" style={{width:"100%",minWidth:320}}>
                        ...
                        <tbody>{result.downloads.map((d,i)=><tr key={i}><td><span className="mono" style={{color:"var(--gray-400)"}}>{i+1}</span></td><td><span className="mono">{fmtBytes(d.bytes)}</span></td><td><span className="mono" style={{color:"#16a34a",fontWeight:600}}>{fmtKbs(d.speedKbs)}</span></td><td><span className="mono" style={{color:"var(--gray-500)"}}>{fmtMs(d.elapsed)}</span></td></tr>)}</tbody>
                      </table>
                    </div>
                  </>
                )}
