...
/**
 * Component for copying and displaying BLS keys
 */
...
/**
 * Fixed right-side tooltip showing provider details
 */
function HoverTooltip({ p }: { p: StorageProvider }) {
  ...
  return (
    <div style={{ position: "fixed", right: 24, top: "50%", transform: "translateY(-50%)", zIndex: 600, width: 288, background: bg, border: `1px solid ${bdr}`, borderRadius: 14, padding: "15px 17px", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", boxShadow: "0 20px 50px rgba(0,0,0,0.25)", pointerEvents: "none" }}>
      <div style={{ fontSize: 10, fontWeight: 700, color: pm, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 5 }}>STORAGE PROVIDER</div>
      <div style={{ fontSize: 14, fontWeight: 700, color: "#3b82f6", fontFamily: "monospace", marginBottom: 10 }}>{p.addressShort}</div>
      
      <div style={{ display: "flex", gap: 5, marginBottom: 12, flexWrap: "wrap" }}>
        {[
          { label: p.state, bg: isH ? "rgba(34,197,94,0.15)" : "rgba(245,158,11,0.15)", color: isH ? "#22c55e" : "#f59e0b", bdr_: isH ? "rgba(34,197,94,0.3)" : "rgba(245,158,11,0.3)" },
          { label: p.health, bg: isH ? "rgba(34,197,94,0.12)" : "rgba(239,68,68,0.12)", color: isH ? "#22c55e" : "#ef4444", bdr_: "none" },
          { label: `Zone ${ZONE_META[p.availabilityZone]?.shortLabel ?? p.availabilityZone.replace("dc_", "")}`, bg: "rgba(245,158,11,0.12)", color: "#f59e0b", bdr_: "none" },
        ].map((b, i) => (
          <span key={i} style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 5, background: b.bg, color: b.color, border: b.bdr_ !== "none" ? `1px solid ${b.bdr_}` : "none" }}>● {b.label}</span>
        ))}
      </div>

      <div style={{ background: "rgba(128,128,128,0.08)", borderRadius: 8, padding: "9px 11px", marginBottom: 10 }}>
        <div style={{ fontSize: 9, color: pm, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 3 }}>LOCATION</div>
        <div style={{ fontSize: 14, fontWeight: 700, color: pt }}>{p.geo?.city ?? ZONE_META[p.availabilityZone]?.label ?? "Unknown"}{p.geo?.countryCode ? `, ${p.geo.countryCode}` : ""}</div>
        {p.geo && <div style={{ fontSize: 10, color: pm, fontFamily: "monospace", marginTop: 2 }}>{p.geo.lat?.toFixed(4)}°, {p.geo.lng?.toFixed(4)}°</div>}
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "5px 10px", marginBottom: bls ? 10 : 0 }}>
        <div><div style={{ fontSize: 9, color: pm, textTransform: "uppercase" }}>ZONE</div><div style={{ fontSize: 12, color: pt, fontWeight: 500 }}>{ZONE_META[p.availabilityZone]?.label ?? p.availabilityZone}</div></div>
        <div><div style={{ fontSize: 9, color: pm, textTransform: "uppercase" }}>CAPACITY</div><div style={{ fontSize: 12, color: pt, fontWeight: 500 }}>{p.capacityTiB != null ? `${p.capacityTiB.toFixed(1)} TiB` : "—"}</div></div>
        <div><div style={{ fontSize: 9, color: pm, textTransform: "uppercase" }}>NET IP</div><div style={{ fontSize: 11, color: pt, fontWeight: 500, fontFamily: "monospace" }}>{p.netAddress ?? "—"}</div></div>
        
        {/* TCP Status in Tooltip */}
        {(p as any).tcpStatus && (p as any).tcpStatus !== "unknown" && (
          <div>
            <div style={{ fontSize: 9, color: pm, textTransform: "uppercase" }}>TCP Status</div>
            <div style={{ fontSize: 12, color: (p as any).tcpStatus === "online" ? "#22c55e" : "#ef4444", fontWeight: 600 }}>
              {(p as any).tcpStatus === "online" ? "● Online" : "○ Offline"}
            </div>
          </div>
        )}
      </div>
      ...
      {bls && (
        <div style={{ borderTop: `1px solid ${bdr}`, paddingTop: 10 }}>
          <div style={{ fontSize: 9, color: pm, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 4 }}>BLS PUBLIC KEY</div>
          <div style={{ display: "flex", alignItems: "center", gap: 5, background: "rgba(128,128,128,0.07)", borderRadius: 5, padding: "4px 7px" }}>
            <span style={{ fontSize: 10, fontFamily: "monospace", color: pm, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{bls.length > 36 ? `${bls.slice(0, 34)}…` : bls}</span>
            <button onClick={copy} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 12, color: copied ? "#22c55e" : pm, flexShrink: 0, pointerEvents: "auto" }}>{copied ? "✓" : "⧉"}</button>
          </div>
          <div style={{ fontSize: 9, color: pm, marginTop: 3, opacity: 0.7 }}>on-chain · storage_provider_registry</div>
        </div>
      )}
    </div>
  );
...
/**
 * Top summary bar with 6 metrics
 */
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                const isH = p.health === "Healthy";
                const tcp = (p as any).tcpStatus;
                
                return (
                  <tr key={p.address || i} style={{ borderBottom: "1px solid var(--border-soft)", background: i % 2 === 0 ? "var(--bg-card)" : "var(--bg-card2)", cursor: "default" }} onMouseEnter={() => setHoveredSP(p)} onMouseLeave={() => setHoveredSP(null)}>
                    <td style={{ padding: "11px 18px", width: 30 }}><div style={{ width: 9, height: 9, borderRadius: "50%", background: isH ? "#22c55e" : "#ef4444", boxShadow: isH ? "0 0 6px #22c55e88" : "0 0 6px #ef444488" }} /></td>
                    <td style={{ padding: "11px 14px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        ...
                        {tcp && tcp !== "unknown" && (
                          <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 6px", borderRadius: 4, background: tcp === "online" ? "rgba(34,197,94,0.12)" : "rgba(239,68,68,0.12)", color: tcp === "online" ? "#22c55e" : "#ef4444", border: `1px solid ${tcp === "online" ? "rgba(34,197,94,0.2)" : "rgba(239,68,68,0.2)"}` }}>
                            {tcp === "online" ? "● TCP" : "○ TCP"}
                          </span>
                        )}
                      </div>
                    ...
                    <td style={{ padding: "11px 14px" }}><div style={{ fontSize: 13, color: "var(--text-secondary)", fontWeight: 500 }}>{ZONE_META[p.availabilityZone]?.label ?? p.availabilityZone}</div></td>
                    ...
                    <td style={{ padding: "11px 18px" }}><BlsKey full={p.fullBlsKey ?? p.blsKey ?? ""} />{p.netAddress && <div style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 1, fontFamily: "monospace" }}>{p.netAddress}</div>}</td>
                  </tr>
                );
              })}
            </tbody>
