"use client";
/**
 * app/page.tsx — Benchmark Page v5.3
 *
 * Changes vs v5.2:
 *  - Adaptive stress test: warm-up 1KB → 64KB → 512KB → 2MB → 5MB → 10MB
 *    Tự động detect breakpoint khi tốc độ drop hoặc timeout
 *  - Custom upload: tối đa 10 MB (1 Shelby chunkset)
 *  - Persistent history: localStorage, KHÔNG có nút clear
 *  - History cũng lưu adaptive benchmark results (maxSuccessfulBytes)
 *  - Mobile responsive: không dùng flex-row cứng, wrap đúng trên điện thoại
 */
...
type Phase =
  | "idle" | "checking" | "latency"
  | "upload" | "download" | "txtime"
  | "done"  | "error";
...
interface BenchResult {
  ...
  tier:                string;
  runAt:               string;
  maxSuccessfulBytes?: number;   // adaptive: largest blob that succeeded
...
// ─── Adaptive sizing ──────────────────────────────────────────────────────────
// 1 Shelby chunkset = 10 MB. Sizes tăng theo bội số 8x để stress test đúng nghĩa.
const ADAPTIVE_SIZES = [
  ...
  2_097_152,     //   2 MB
  5_242_880,     //   5 MB
  10_485_760,    //  10 MB  — 1 full Shelby chunkset
];

const STANDARD_SIZES = [1_024, 10_240, 102_400]; // giữ cho quick run

const MAX_CUSTOM_BYTES = 10 * 1024 * 1024; // 10 MB

const SIZE_LABEL = (b: number) => {
...
// ─── Scoring (v5.3) ──────────────────────────────────────────────────────────
// max=1000, cân bằng 4 thành phần
function calcScore(r: {
  avgUploadKbs: number; avgDownloadKbs: number;
  latency: LatResult;  tx: TxResult;
...
function ProgressBar({ phase, running }: { phase: Phase; running: boolean }) {
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8, fontSize: 12, color: "var(--gray-400)", fontFamily: "var(--font-mono)" }}>
        ...
        {STEPS.map(s => {
          ...
          return (
            <div key={s.phase} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3, flex: 1, opacity: done || current ? 1 : 0.3 }}>
              ...
              <span style={{ fontSize: 8, color: current ? "var(--net-color)" : "var(--gray-400)", fontFamily: "var(--font-mono)", textAlign: "center" }}>
                {s.label}
              </span>
            </div>
          );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
      <svg width={110} height={110} viewBox="0 0 110 110">
        ...
        <text x={55} y={65} textAnchor="middle" fill="#9ca3af" fontSize={9}  fontFamily="var(--font-sans)">{tier.toUpperCase()}</text>
      </svg>
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      ...
      <div style={{ height: 4, background: "var(--gray-100)", borderRadius: 3, overflow: "hidden" }}>
        <div style={{ height: "100%", width: `${Math.min(100, (value / max) * 100)}%`, background: color, borderRadius: 3, transition: "width 0.8s cubic-bezier(.4,0,.2,1)" }} />
      </div>
    </div>
  );
}
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
      <a href={`https://explorer.aptoslabs.com/txn/${hash}?network=${network}`} target="_blank" rel="noreferrer"
        style={{ color: "var(--info, #2563eb)", fontSize: 12, fontFamily: "var(--font-mono)", textDecoration: "none" }}
        title={hash}>
        {hash.slice(0, 8)}…{hash.slice(-5)} ↗
      </a>
      <button onClick={copy} title="Copy hash" style={{ background: "none", border: "none", cursor: "pointer", padding: "0 2px", fontSize: 11, color: copied ? "#16a34a" : "var(--gray-400)" }}>
        {copied ? "✓" : "⎘"}
      </button>
    </span>
  );
  ...
  return (
    <div className="card" style={{ marginBottom: 16 }}>
          ...
          <div className="card-subtitle">
            {loading ? "Checking…"
             : result ? result.ready
               ...
               : `⚠ ${result.failCount} issue(s) found`
      ...
      <div className="card-body" style={{ padding: "0 20px 16px" }}>
        {loading && !result && <div style={{ padding: "14px 0" }}><div className="skeleton" style={{ height: 120, borderRadius: 6 }} /></div>}
        {result?.checks && (
...
function CustomUploadCard({ running, network }: { running: boolean; network: string }) {
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!f) return;
    if (f.size > MAX_CUSTOM_BYTES) { setErr(`File quá lớn — tối đa ${fmtBytes(MAX_CUSTOM_BYTES)} (1 Shelby chunkset)`); return; }
    ...
    <div className="card" style={{ marginBottom: 16 }}>
      <div className="card-header">
        <div>
          ...
          <div className="card-subtitle">File của bạn — tối đa {fmtBytes(MAX_CUSTOM_BYTES)} (1 Shelby chunkset)</div>
        </div>
      </div>
      <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <div onClick={() => inputRef.current?.click()} style={{
            ...
            background: file ? "#f0fdf4" : "#fafafa", transition: "border-color 0.2s",
          }}
...
/** History table — persistent, không có nút xóa */
function HistoryTable({ history }: { history: HistoryEntry[] }) {
  if (!history.length) return (
    <div className="card" style={{ padding: "40px 24px", textAlign: "center" }}>
      <div style={{ fontSize: 28, marginBottom: 10 }}>📊</div>
      <div style={{ fontSize: 14, color: "var(--gray-400)" }}>Chưa có dữ liệu — chạy benchmark để bắt đầu thu thập</div>
    </div>
  );
    ...
    <div className="card">
      <div className="card-header">
        <div>
          ...
          <div className="card-subtitle">{history.length} lần chạy được lưu trữ</div>
        </div>
      </div>
      <div className="card-body" style={{ padding: 0, overflowX: "auto" }}>
        <table className="data-table" style={{ width: "100%", minWidth: 560 }}>
          <thead>
            <tr>
              <th>#</th><th>Mode</th><th>Score</th><th>Tier</th>
              <th>Upload</th><th>Download</th><th>Latency</th><th>TX Confirm</th>
              <th>Max size</th><th>Ran at</th>
            </tr>
          </thead>
          <tbody>
            {history.slice().reverse().map(h => {
              ...
              return (
                <tr key={h.id}>
                  ...
                  <td><span style={{ fontSize: 10, fontWeight: 600, color: h.mode === "adaptive" ? "#2563eb" : "#9333ea", textTransform: "uppercase", letterSpacing: "0.05em" }}>{h.mode}</span></td>
                  <td><span className="mono" style={{ fontWeight: 700, color }}>{h.score}</span></td>
                  <td><span style={{ fontSize: 11, color, fontWeight: 600 }}>{h.tier}</span></td>
...
export default function BenchmarkPage() {
  ...
  const saveHistory = useCallback((res: BenchResult) => {
    setHistory(prev => {
  ...
  const runDiagnose = useCallback(async () => {
    ...
    try {
      ...
      setDiagnose({ ready: false, passCount: 0, failCount: 1, warnCount: 0, checks: [{ name: "Backend API", status: "fail", hint: e.message }], summary: "Không kết nối được Backend" });
  ...
  const run = useCallback(async () => {
    ...
    try {
      // 1. Balance
      addLog("— [1/5] Kiểm tra ví…");
      const bal = await call<Balance>("/api/benchmark/balance");
      setBalance(bal);
      if (myRun !== runIdRef.current) return;
      if (!bal.ready) throw new Error("Số dư không đủ — nhấn 'Request faucet'");
      addLog(`✓ APT: ${bal.apt.toFixed(4)} · ShelbyUSD: ${bal.shelbyusd.toFixed(6)}`);

      // 2. Latency
      setPhase("latency");
      addLog("— [2/5] Đo latency blockchain (5 ping)…");
      ...
      // 3. Uploads (adaptive or standard)
      setPhase("upload");
      ...
      if (benchMode === "adaptive") {
        ...
        for (let i = 0; i < ADAPTIVE_SIZES.length; i++) {
          ...
          const label = SIZE_LABEL(bytes);
          addLog(`  ↑ [${i + 1}/${ADAPTIVE_SIZES.length}] ${label}…`);
          try {
            const u = await call<UpResult>("/api/benchmark/upload", { sizeIndex: -1, adaptiveBytes: bytes });
            if (myRun !== runIdRef.current) return;
            uploads.push(u);
            maxSuccessfulBytes = bytes;
            addLog(`  ✓ ${label}: ${fmtKbs(u.speedKbs)} in ${fmtMs(u.elapsed)}${u.txHash ? ` · tx ${u.txHash.slice(0, 8)}…` : ""}`);
          } catch (e: any) {
            addLog(`  ✗ ${label}: ${e.message} — stopping here`);
            break; // stop at first failure (bandwidth/timeout limit found)
          }
        }
        if (uploads.length === 0) throw new Error("Tất cả upload đều thất bại");
        addLog(`✓ Adaptive done — max successful: ${SIZE_LABEL(maxSuccessfulBytes)}`);
      } else {
        // Standard: 3 sizes cố định
        addLog("— [3/5] Upload 3 payload cố định (1KB / 10KB / 100KB)…");
        for (let i = 0; i < 3; i++) {
          const u = await call<UpResult>("/api/benchmark/upload", { sizeIndex: i });
          if (myRun !== runIdRef.current) return;
          uploads.push(u);
          maxSuccessfulBytes = STANDARD_SIZES[i];
          addLog(`  ✓ ${SIZE_LABEL(STANDARD_SIZES[i])}: ${fmtKbs(u.speedKbs)} · ${fmtMs(u.elapsed)}`);
        }
      }

      // 4. Downloads
      setPhase("download");
      addLog("— [4/5] Download blobs…");
      const downloads: DlResult[] = [];
      // download 3 blobs tối đa (để không quá lâu)
      const toDownload = uploads.slice(-3).filter(u => u.blobName);
      for (const up of toDownload) {
        try {
          ...
          addLog(`  ✗ download: ${e.message}`);
        }
      }

      // 5. TX time
      setPhase("txtime");
      addLog("— [5/5] Đo Aptos transaction timing…");
      ...
      setPhase("done");
      saveHistory(res);
      addLog(`— Done · Score: ${score}/1000 (${tier}) · Max blob: ${SIZE_LABEL(maxSuccessfulBytes)}`);
  ...
  const requestFaucet = useCallback(async () => {
    addLog("— Yêu cầu token từ faucet…");
    try {
      await call("/api/benchmark/faucet", {});
      addLog("✓ Faucet requested — balance cập nhật sau ~5s");
    ...
    <div>
      <style>{`
        ...
        @media (max-width: 600px) {
          .bench-result-grid { flex-direction: column !important; }
          .bench-score-ring  { align-self: center; }
        }
      `}</style>
      ...
      <div className="page-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10, marginBottom: 16 }}>
        <div>
          ...
          <p className="page-subtitle" style={{ fontSize: 13 }}>
            Upload · download · latency · TX speed trên <strong>{config.label}</strong>
          </p>
        </div>
        {/* Tab switcher */}
        <div style={{ display: "flex", background: "#f4f4f4", borderRadius: 10, padding: 3, gap: 2, alignSelf: "flex-start" }}>
        ...
        <>
          ...
          <div className="card" style={{ marginBottom: 16, padding: "18px 20px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
              ...
              {(["adaptive", "standard"] as const).map(m => (
                <button key={m} onClick={() => setBenchMode(m)} disabled={running} style={{
                  padding: "5px 14px", borderRadius: 8, fontSize: 12, fontWeight: benchMode === m ? 600 : 400,
                  ...
                  {m === "adaptive" ? "🔬 Adaptive stress" : "⚡ Quick (3 sizes)"}
                </button>
              ))}
              <span style={{ fontSize: 11, color: "var(--gray-400)", marginLeft: 4 }}>
                {benchMode === "adaptive"
                  ? "1KB → 64KB → 512KB → 2MB → 5MB → 10MB — tự tìm breakpoint"
                  : "1KB / 10KB / 100KB — nhanh hơn, phù hợp kiểm tra nhanh"}
              </span>
            </div>
            ...
            <button
              ...
              style={{ width: "100%", padding: "13px 0", fontSize: 15, justifyContent: "center", borderRadius: 12, marginBottom: running || phase !== "idle" ? 16 : 0, opacity: (!diagnose?.ready && !running) ? 0.6 : 1 }}
            >
              {running
                ? `Running — ${STEPS.find(s => s.phase === phase)?.label ?? phase}…`
                : !diagnose?.ready
            ...
            <div className="card" style={{ marginBottom: 16 }}>
              <div className="card-header">
                <div>
                  ...
                  <div className="card-subtitle">{config.label} · {result.runAt} · mode: {result.mode}</div>
                </div>
              </div>
              <div className="card-body">
                {/* Score + summary row */}
                <div className="bench-result-grid" style={{ display: "flex", gap: 24, alignItems: "flex-start", flexWrap: "wrap", marginBottom: 24 }}>
                  <div className="bench-score-ring">
                    <ScoreRing score={result.score} tier={result.tier} />
                  </div>
                  <div style={{ flex: 1, minWidth: 200 }}>
                    {result.maxSuccessfulBytes && result.mode === "adaptive" && (
                      <div style={{ display: "inline-flex", alignItems: "center", gap: 6, marginBottom: 12, padding: "4px 10px", borderRadius: 6, background: "#eff6ff", border: "1px solid #bfdbfe" }}>
                        <span style={{ fontSize: 11, color: "#1d4ed8", fontWeight: 600 }}>Max blob size:</span>
                ...
                <div style={{ overflowX: "auto" }}>
                  <table className="data-table" style={{ width: "100%", minWidth: 460 }}>
